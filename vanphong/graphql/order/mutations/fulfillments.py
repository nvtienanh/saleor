...
class FulfillmentRefundAndReturnRoomBase(BaseMutation):
    class Meta:
        abstract = True
    ...
    def clean_fulfillment_lines(
        ...
        for line, line_data in zip(fulfillment_lines, fulfillment_lines_data):
            ...
            if replace and not line.order_line.variant_id:
                cls._raise_error_for_line(
                    "Unable to replace line as the assigned room doesn't exist.",
                    "OrderLine",
                    line.pk,
                    "order_line_id",
                )
...
class FulfillmentRefundRooms(FulfillmentRefundAndReturnRoomBase):
...
class OrderReturnRoomsInput(graphene.InputObjectType):
    order_lines = graphene.List(
...
class FulfillmentReturnRooms(FulfillmentRefundAndReturnRoomBase):
    ...
    replace_order = graphene.Field(
        Order,
        description="A draft order which was created for rooms with replace flag.",
    )
    ...
    class Arguments:
        ...
        input = OrderReturnRoomsInput(
            required=True,
            description="Fields required to return rooms.",
        )
