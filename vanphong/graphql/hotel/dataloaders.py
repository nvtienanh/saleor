...
class AvailableQuantityByRoomVariantIdAndCountryCodeLoader(
    ...
    context_key = "stock_by_roomvariant_and_country"
    ...
    def batch_load(self, keys):
        ...
        # For each country code execute a single query for all room variants.
    ...
    def batch_load_country(
        ...
        results = Stock.objects.filter(room_variant_id__in=variant_ids)
        if country_code:
            results.filter(hotel__shipping_zones__countries__contains=country_code)
        results = results.annotate_available_quantity()
        results = results.values_list(
            "room_variant_id", "hotel__shipping_zones", "available_quantity"
        )
        ...
        quantity_by_shipping_zone_by_room_variant: DefaultDict[
            int, DefaultDict[int, int]
        ] = defaultdict(lambda: defaultdict(int))
        for variant_id, shipping_zone_id, quantity in results:
            quantity_by_shipping_zone_by_room_variant[variant_id][
                shipping_zone_id
            ] += quantity
        ...
        ) in quantity_by_shipping_zone_by_room_variant.items():
