...
variables are set (see the `vanphong.core.utils.random_data.configure_braintree` function
for more details)
...
ROOT_URLCONF = "vanphong.demo.urls"

PLUGINS += ["vanphong.plugins.anonymize.plugin.AnonymizePlugin"]

GRAPHENE["MIDDLEWARE"].insert(0, "vanphong.graphql.middleware.ReadOnlyMiddleware")  # type: ignore
...
PWA_ORIGINS = get_list(os.environ.get("PWA_ORIGINS", "demo.vanphong.io"))
PWA_DASHBOARD_URL_RE = re.compile("^https?://[^/]+/dashboard/.*")

ROOT_EMAIL = os.environ.get("ROOT_EMAIL")

# Remove "vanphong.core" and add it after adding "vanphong.demo", to have "populatedb"
# command overriden when using demo settings
# (see vanphong.demo.management.commands.populatedb).
INSTALLED_APPS.remove("vanphong.core")
INSTALLED_APPS += ["vanphong.demo", "vanphong.core"]
...
def before_send(event: dict, _hint: dict):
    ...
    if ev_logger_name != "vanphong.graphql.errors.handled":
        return event
